//selecting the result container div, so that we can append favourite Movies inside it
...
if (favMovies.length === 0) {
  ...
  emptyMessageContainer.innerHTML = "<p><h2>No favourite movies added yet !!! Please Add it..</h2></p>";
  resultContainer.appendChild(emptyMessageContainer);
} else {
  //Get all Favourite movies
  favMovies?.forEach((id) => {
    getData(id); // Get Movie from API key ID
  });
}
...
async function getData(movieID) {
  ...
  const movieDetails = await result.json(); //Make data in readable format - JSON
...
const AddMovie = (details) => {
  ...
  btn.innerHTML = `<i data-id="${details.imdbID}" class="fa-solid fa-trash">`; //Set unique id to delete exact movie Details
...
const deleteMovie = (e) => {
  ...
  //Add new data of favourite Movies to localstorage
  localStorage.setItem("favMovies", JSON.stringify(favMovies));

  // Display empty favorite list message if all favorites are deleted
  if (favMovies.length === 0) {
    showEmptyMessage();
  }
};

// Function to display empty favorite list message
